...
import { Pager } from "@/components/pageShell";
  ...
  return (
    <>
      ...
      <div className="mt-6 border border-border bg-card/40">
        ...
        {loading ? (
          <div className="p-8 text-center font-mono text-xs text-muted-foreground">Loading transmission…</div>
        ) : error ? (
          <div className="p-8 text-center font-mono text-xs text-destructive">Signal lost — {error}</div>
        ) : players.length === 0 ? (
          <div className="p-8 text-center font-mono text-xs text-muted-foreground">No scores yet on this frequency.</div>
        ) : (
          <ul>
            {players.map((p, i) => {
              ...
              return (
                ...
                >
                  <span className="font-display text-sm text-secondary">{String(rank).padStart(2, "0")}</span>
                  <Link href={`/player/${p.playerId}`} className="font-mono text-sm text-foreground hover:text-secondary">
                    {name}
                    {p.tag && (<span className="ml-2 text-muted-foreground">#{p.tag}</span>)}
                  </Link>
                  <span className="text-right font-mono text-xs text-muted-foreground">—</span>
                  <span className="text-right font-mono text-sm">{score.toLocaleString()}</span>
                </motion.li>
              );
            })}
          </ul>
